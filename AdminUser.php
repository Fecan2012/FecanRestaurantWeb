...
class AdminUser{
	...
	function authenticate($username, $password){
		...
		$userID = $wpeaterydao->getUserID($this->username, $this->password);
		if(!$wpeaterydao->hasMysqlError()){
			//password_verify accepts two parameters: the user's password, and
			//will return true. Otherwise, it will return false.
			if($userID == '1'){   // when the username is 'admin' and the password is 'passme', AdminID is '1' and then return true to log in.
